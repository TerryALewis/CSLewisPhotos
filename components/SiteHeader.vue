      ...
      <div class="flex justify-between h-16 items-center">
        ...
        <div class="hidden md:flex items-center space-x-6">
          <div class="relative">
            <button
              type="button"
              :class="headerTextClass + ' inline-flex items-center px-3 py-2 text-sm font-medium hover:text-gray-200'"
        ...
        <div class="flex items-center md:hidden">
          <button @click="showMobileMenu = !showMobileMenu" class="p-2 rounded-md focus:outline-none" :aria-expanded="showMobileMenu" aria-label="Open menu">
        ...
        <div v-if="showMobileMenu" class="fixed inset-0 z-40 md:hidden">
          ...
          <div class="absolute inset-0 bg-black bg-opacity-20" @click="showMobileMenu = false"></div>
          <!-- panel anchored under header at top-right -->
          <div class="absolute top-16 right-4 w-72 bg-white/70 backdrop-blur-sm rounded-lg shadow-xl p-4 max-h-[60vh] overflow-auto">
            <div class="flex items-center justify-between">
              <h2 class="text-lg font-semibold">Menu</h2>
              <button @click="showMobileMenu = false" class="text-gray-600">Close</button>
            </div>
            <nav class="mt-4 flex flex-col space-y-2">
              <button @click="showGalleries = !showGalleries" class="text-left font-medium">Galleries</button>
              <ul v-if="showGalleries" class="pl-4">
                <li v-for="g in galleriesLinks" :key="g.id">
                  <NuxtLink :to="g.href" class="block py-2 text-sm text-gray-700" @click="showMobileMenu = false">{{ g.title }}</NuxtLink>
                </li>
              </ul>
              ...
              <ClientOnly>
                <template v-if="isSignedInBool">
                  ...
                  <button @click="showMobileAccount = !showMobileAccount" class="text-left font-medium">Welcome{{ userFirstName ? ', ' + userFirstName : '' }}</button>
                  <ul v-if="showMobileAccount" class="pl-4">
                    <li>
                      <NuxtLink to="/account" class="block py-2 text-sm text-gray-700" @click="showMobileMenu = false">Account</NuxtLink>
                    </li>
                    <li>
                      <div class="py-2">
                        <SignOutButton class="w-full text-left">Sign out</SignOutButton>
                ...
                <template v-else>
                  <SignInButton class="block w-full text-left py-2">Sign In</SignInButton>
                </template>
              </ClientOnly>

              <NuxtLink to="/" class="block py-2 text-sm text-gray-700" @click="showMobileMenu = false">Home</NuxtLink>
              <NuxtLink to="/cart" class="block py-2 text-sm text-gray-700" @click="showMobileMenu = false">Cart ({{ cartCount }})</NuxtLink>
            </nav>
          </div>
        </div>
      </div>
